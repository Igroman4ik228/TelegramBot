...
def main_build(replace_lesson, day_of_week):
    default_lesson = [
    ...
    beautiful_lesson = create_beautiful_lesson(current_lesson, day_of_week)
    write_to_database(beautiful_lesson, day_of_week)
...
def create_beautiful_lesson(current_lesson, day_of_week):
    beautiful_lesson = f"Расписание на {day_of_week}: \n"
    for lesson in current_lesson:
        if lesson[1]!= None:
            beautiful_lesson += f"`{lesson[0]}.` **{lesson[1]}**\n"
    return beautiful_lesson

def write_to_database(final_lesson, day_of_week):
    logger.info(f"Записываем расписание в базу данных на {day_of_week}: {final_lesson}")